...
class Row:
    ...
    def __str__(self):
        return f"id:{self.id}, name:{self.fname}, lastname:{self.lname}, state:{self.state}"
...
class Report:
    ...
    def get_number_of_pages(self):
        ...
        """
        return math.ceil(len(self.rows) / self.limit)
    ...
    def get_paged_rows(self, sort_field, page):
        ...
        if sort_field is not None:
            if sort_field[0] == '-':
                ...
                reversed = False
            self.rows.sort(key=lambda x: getattr(x, sort_field), reverse=reversed)
        return self.rows[(page - 1) * self.limit: (page) * self.limit]
