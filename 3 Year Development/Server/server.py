...
class Player:
    ...
    def get_packet_announce(self):
        return struct.pack("II3f", 1, self.player_id, *self.transform) + self.name

    def get_packet_transform(self):
        return struct.pack("II3f", 0, self.player_id, *self.transform)
...
class Server:
        ...
        for dead_peer in dead_clients:
            for peer in self.known_clients:
                ...
                if current_player.peer == dead_peer:
                    continue
                self.socket.sendto(self.known_clients[dead_peer].get_packet_death(), current_player.peer,)
    ...
    def process_message(self, packet, sender):
        ...
        if command_id == 0:
            player_id, x, y, yaw = struct.unpack("Ifff", packet[4:20])
